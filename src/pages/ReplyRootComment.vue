  ...
  <div class="compose">
    ...
    <div class="content-preview">
      <textarea-autosize @focus.native="textareaGetFocus"
                         rows="3"
                         ref="textarea"
                         v-model="content[$route.params.rootCommentID]" :placeholder="placeholder"/>
      <van-uploader
        v-show="showPreview"
        ref="uploader"
        max-count="1"
...
export default {
  ...
  computed: {
    ...
    safeContent () {
      if (this.content[this.$route.params.rootCommentID]) {
        return this.content[this.$route.params.rootCommentID].replace(/[<|>]/g, '')
      } else {
        return this.content[this.$route.params.rootCommentID]
      }
    }
  },
  watch: {
  ...
  methods: {
    ...
    newRecord () {
      ...
      let content = this.content[this.$route.params.rootCommentID]
        ...
        if (!userID) {
          ...
          if (!this.isSameTimeShare) {
            addComment(formData).then(res => {
              this.content[this.$route.params.rootCommentID] = ''
              this.showEmojiBar = false
    ...
    confirmEmotion (value) {
      if (window.getSelection) {
        ...
        this.content[this.$route.params.rootCommentID] = this.textArea.value
    ...
    getRootCommentDetail () {
      getCommentDetail(this.$route.params.rootCommentID).then(res => {
        ...
        this.placeholder = `@${this.rootCommentDetail.user.nickname}`
      })
    }
  }
}
